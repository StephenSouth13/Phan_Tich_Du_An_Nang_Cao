...
Recall   : {r.recall:.4f}
F1-score : {r.f1:.4f}
ROC-AUC  : {r.roc_auc:.4f if r.roc_auc is not None else 'N/A'}
...
"""
            body = body.replace("{r.roc_auc:.4f if r.roc_auc is not None else 'N/A'}", f"{r.roc_auc:.4f}" if r.roc_auc is not None else "N/A")
